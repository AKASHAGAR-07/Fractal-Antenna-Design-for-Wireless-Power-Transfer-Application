...
def create_solid_hexagon(name, center_x, center_y, radius, z_pos, thickness):
    ...
    point_str_list = []
    for pt in pts:
        point_str_list.append(f"CoordinateSystemID:= 1, X:= {pt[0]}mm, Y:= {pt[1]}mm, Z:= {z_pos}mm")
    # Add first point to close polyline
    point_str_list.append(f"CoordinateSystemID:= 1, X:= {pts[0][0]}mm, Y:= {pts[0][1]}mm, Z:= {z_pos}mm")
    ...
    oEditor.CreatePolyline(
        ...
        [
            ...
            "Name:=", name,
            "Flags:=", "",
            "Color:=", "(218 165 32)",  # Gold/Copper color
            "Transparency:=", 0,
            "MaterialName:=", "copper",
            "SolveInside:=", False
        ]
...
def build_geometry():
    ...
    if not IN_HFSS:
        ...
        print(f"  - Substrate dimensions: {SUBSTRATE_X} x {SUBSTRATE_Y} x {SUBSTRATE_Z} mm (FR-4)")
        print(f"  - Hexagonal Patch outer radius: {PATCH_RADIUS_E} mm (Copper)")
        print(f"  - Microstrip feedline width: {FEED_WIDTH} mm, length: {FEED_LENGTH} mm")
    ...
    oEditor.CreateBox(
        [
            "NAME:BoxParameters",
            "XStart:=", f"-{SUBSTRATE_X/2}mm",
            "YStart:=", "0mm",
            "ZStart:=", "0mm",
            "XSize:=", f"{SUBSTRATE_X}mm",
            "YSize:=", f"{SUBSTRATE_Y}mm",
            "ZSize:=", f"{SUBSTRATE_Z}mm"
        ],
        [
            "NAME:Attributes",
            "Name:=", "Substrate",
            "Flags:=", "",
            "Color:=", "(0 128 0)",  # Green FR-4
            "Transparency:=", 0.4,
    ...
    oEditor.CreateBox(
        [
            ...
            "XStart:=", f"-{SUBSTRATE_X/2}mm",
            "YStart:=", "0mm",
            "ZStart:=", f"-{GROUND_THICKNESS}mm",
            "XSize:=", f"{SUBSTRATE_X}mm",
            "YSize:=", f"{SUBSTRATE_Y}mm",
            "ZSize:=", f"{GROUND_THICKNESS}mm"
        ],
        [
            "NAME:Attributes",
            "Name:=", "GroundPlane",
    ...
    oEditor.CreateBox(
        [
            "NAME:BoxParameters",
            "XStart:=", f"-{FEED_WIDTH/2}mm",
            "YStart:=", "0mm",
            "ZStart:=", f"{SUBSTRATE_Z}mm",
            "XSize:=", f"{FEED_WIDTH}mm",
            "YSize:=", f"{FEED_LENGTH}mm",
            "ZSize:=", f"{COPPER_THICKNESS}mm"
        ],
        [
            "NAME:Attributes",
            "Name:=", "Feedline",
    ...
    patch_center_y = 25.0
    create_solid_hexagon("MainPatch", 0.0, patch_center_y, PATCH_RADIUS_E, SUBSTRATE_Z, COPPER_THICKNESS)

    # 5. Create Iteration 1 Central Slot (to subtract)
    # Scale slot by 1/3
    slot1_radius = PATCH_RADIUS_E / 3.0
    create_solid_hexagon("Slot1", 0.0, patch_center_y, slot1_radius, SUBSTRATE_Z, COPPER_THICKNESS)

    # Subtract Slot1 from MainPatch
    oEditor.Subtract(
        [
            "NAME:Selections",
            "BlankParts:=", "MainPatch",
            "ToolParts:=", "Slot1"
        ],
        [
            "NAME:SubtractParameters",
            "ReplaceWithParts:=", True
        ]
    )
    ...
    vertices = get_hexagon_vertices(0.0, patch_center_y, slot1_radius * 1.5) # placed intermediate to edges
    
    slot_names = []
    for idx, vert in enumerate(vertices):
        slot_name = f"Slot2_{idx}"
        create_solid_hexagon(slot_name, vert[0], vert[1], slot2_radius, SUBSTRATE_Z, COPPER_THICKNESS)
    ...
    oEditor.Unify(
        [
            "NAME:Selections",
            "Selections:=", "Feedline,MainPatch"
        ],
        [
            "NAME:UnifyParameters",
            "Glue:=", False
        ]
    )
