...
def plot_return_loss(output_dir):
    ...
    plt.axhline(-10, color='gray', linestyle='--', linewidth=1.0, label='Standard Limit (-10 dB)')
    plt.axvline(2.40, color='#2c3e50', linestyle=':', linewidth=1.2, label='Resonant Peak (2.4 GHz)')
    
    # Highlighting target data point
    plt.plot(2.40, -44.0, 'o', color='#2c3e50', markersize=8)
    plt.annotate('Resonance: 2.40 GHz\nS11 = -44.0 dB', 
                 xy=(2.40, -44.0), 
                 xytext=(1.5, -35.0),
...
def plot_vswr(output_dir):
    ...
    s11_total = np.minimum(s11_total, -0.1)  # Prevent divide by zero / reflection coeff >= 1
    ...
    plt.axhline(2.0, color='gray', linestyle='--', linewidth=1.0, label='Acceptable Limit (2.0)')
    plt.axvline(2.40, color='#2c3e50', linestyle=':', linewidth=1.2, label='Resonant Peak (2.4 GHz)')
    
    plt.plot(2.40, 1.13, 'o', color='#2c3e50', markersize=8)
    plt.annotate('VSWR = 1.13 at 2.4 GHz', 
                 xy=(2.40, 1.13), 
                 xytext=(2.7, 4.0),
                 arrowprops=dict(facecolor='black', shrink=0.08, width=1, headwidth=6))
...
def plot_radiation_patterns(output_dir):
    ...
    ax.plot(theta, e_plane_gain, color='#1abc9c', linewidth=2.0, label='E-Plane (Electric Field)')
    ...
    fig = plt.figure(figsize=(6, 6))
    ax = fig.add_subplot(111, polar=True)
    ax.plot(theta, h_plane_gain, color='#9b59b6', linewidth=2.0, label='H-Plane (Magnetic Field)')
